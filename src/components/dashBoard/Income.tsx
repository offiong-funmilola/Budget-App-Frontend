...
import { ActionType, Option } from "../../type";
...
function Income() {
  const { dispatch, record } = useBudgetContext()
  const {postReq} = useClientRequestContext()
   
  const [incomeValues, setIncomeValues] = useState<Income>({ source: "", amount: 0 });
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const form = e.currentTarget as HTMLFormElement;
    const error = validateIncome(incomeValues);
    if (error.message !== "") {
      toast.warn(error.message);
      return;
    }
    dispatch({ type: ActionType.addIncome, payload: incomeValues });
    form.reset();
    setIncomeValues({ ...incomeValues, source: "", amount: 0 });
  };
  ...
  const handleIncomeUpdate = async (e: any) => {
    ...
    try {
      ...
      setIncomeValues({ source: "", amount: 0 });
    }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-2 gap-4 bg-white border rounded-2xl">
      <form
        className="w-full md:w-3/4 lg:w-1/2 flex-col gap-3"
        onSubmit={handleRecordCreation}
      >
        <div className="w-full flex gap-3">
          <div className="w-1/2">
            <Select
              options={options}
              name="month"
              onChange={(choice) => setData({ ...data, month: choice?.value })}
          ...
          <div className="w-1/2">
            <Select
              options={yearOptions}
              name="year"
              onChange={(choice) => setData({ ...data, year: choice?.value })}
              className="w-full border border-gray-500 rounded-md px-3 py-4"
            />
          </div>
        </div>
        <div className="w-1/2 flex flex-col gap-1">
          ...
          <input
            ...
            type="number"
            onChange={(e) =>
              setData({ ...data, saving: Number(e.target.value) })
            }
            className="w-full border border-gray-500 rounded-md px-3 py-4"
          />
        </div>
        <div className="w-full flex items-center justify-center mt-1">
          <button
            type="submit"
            className=" bg-purple-900 text-white p-3  rounded-full"
          >
            Submit
          </button>
        </div>
      </form>
      <h4 className="text-2xl font-sans font-bold">
        Enter Your Source of Income
      </h4>
      <form
        className="w-full md:w-3/4 lg:w-1/2 h-1/2 border border-purple-900 py-5 px-10 flex flex-col gap-3 rounded-2xl bg-white"
      ...
      >
        <div className="w-full flex flex-col gap-1 text-lg">
          ...
          <Select
            ...
            name="source"
            onChange={(choice) =>
              setIncomeValues({ ...incomeValues, source: choice?.value })
            }
          />
        </div>
        <div className="w-full flex flex-col gap-1 text-lg">
          ...
          <input
            ...
            onChange={(e) =>
              setIncomeValues({ ...incomeValues, amount: Number(e.target.value) })
            }
          />
        </div>
